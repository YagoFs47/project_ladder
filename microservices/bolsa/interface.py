...
class BolsaAposta:
    ...
    def refresh_token(self, tokens_model: SessionsBolsaApostaModel):
        ...
        if refresh.status_code == HTTPStatus.FORBIDDEN:
            ...
            self.delete_muiltiple_cookies()
            token = SessionsBolsaApostaModel.objects.first()
            token.biab_customer=self.client.cookies.get("BIAB_CUSTOMER")
            token.authorization=self.client.cookies.get("Authorization")
            token.sb=self.client.cookies.get("sb")
            token.save()
            return True
    ...
    def verify_tokens(self):
        ...
        if SessionsBolsaApostaModel.objects.count() > 0: #EXISTE
            ...
            elif test_session.status_code == 401:
                print("\033[31mOs cookies não são válidos\033[m")
                if self.refresh_token(tokens_model):
                    return True
